...
export function AppShell({ children, view, onView, profile, analyses, activeAnalysisId, onAnalysis, settings, loading, onSettings, onLogout, professional }: {
  children: ReactNode;
  view: ViewId;
  ...
  const navigation = professional ? [...views, ["backoffice", "Backoffice"] as const] : views;
  return <div className="app-shell">
    ...
    <nav className="primary-nav" aria-label="Navegação principal">{navigation.map(([id, label]) => <button type="button" key={id} className={view === id ? "active" : ""} aria-current={view === id ? "page" : undefined} onClick={() => onView(id)}>{label}</button>)}</nav>
